...
RSpec.describe Applitools::Calabash::EyesCalabashAndroidScreenshot do
  it 'Android densities table' do
    expect(described_class::ANDROID_DENSITY.keys).to contain_exactly(120, 160, 213, 240, 320, 480)
  ...
  describe 'density & scale_factor' do
    ...
    it 'sets scale_factor from options' do
      new_instance = described_class.new(Applitools::Screenshot.from_image(::ChunkyPNG::Image.new(5, 5)), scale_factor: 1.5)
      expect(new_instance.scale_factor).to eq 1.5
    end

    it 'rejects invalid density' do
      expect { described_class.new(Applitools::Screenshot.from_image(::ChunkyPNG::Image.new(5, 5)), density: 242) }.to raise_error Applitools::EyesError
    end
  ...
  describe 'convert_region_location' do
    ...
    it 'from DRIVER to SCREENSHOT_AS_IS' do
      expect(subject.convert_region_location(non_empty_region, described_class::DRIVER, described_class::SCREENSHOT_AS_IS)).to match_region(non_empty_region)
    end
    ...
    it 'from CONTEXT_RELATIVE to SCREENSHOT_AS_IS' do
      ...
      expect(subject.convert_region_location(non_empty_region, described_class::CONTEXT_RELATIVE, described_class::SCREENSHOT_AS_IS)).to match_region(scaled_region)
    end
